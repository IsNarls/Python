...
class NPC_Crime(pg.sprite.Sprite):
    ...
    def __init__(self):
        ...
        # Actions
        self.actions = ['work', 'idle', 'sleep', 'Get Groceries', 'Go_to_convience_store', 'Eat'] 
        self.avaliable_actions = self.actions #'work',
   #Q value block     
   ...
   #status checks 
    ...
    def update_avaliable_actions(self):
        if self.Min_work_time <= self.Current_Hour < self.Max_work_time and self.unemployed != False:
            self.avaliable_actions = ['work', 'idle', 'Get Groceries', 'Go_to_convience_store', 'Eat']
        elif self.Current_Hour >= 17 or self.Current_Hour < 8: 
            self.avaliable_actions = ['sleep', 'idle', 'Get Groceries', 'Go_to_convience_store', 'Eat']   
...
#Task completion section
    ...
    def Write_location_to_file(self):
            #Location = (int(self.current_position[0]), int(self.current_position[1]))
            #self.movement_list.append(Location)
            #print(len(self.movement_list))
            #if len(self.movement_list) == 10000: 
            with file_lock:
                Location = (int(self.current_position[0])+self.color, int(self.current_position[1]))
    ...
    def update(self):
        while self.is_alive == True:
            ...
            if self.action_lock == True:
                if self.action == 'work':
                    self.color = 10000
                    self.Do_Task(self.work(),self.WORK_DURATION,self.work_location,'work')
                    if self.success == 2:
                    ...
                    self.color = 20000
                    self.Do_Task(self.Get_Groceries(),self.shopping_duration,self.Grocery_location,'get Groceries')
                    if self.success == 2:
                        self.reward = self.age + self.net_worth + self.energy + self.hunger
                        self.update_q_value(self.action, self.reward)
                        self.color = 0
                elif self.action == 'sleep':
                    self.Do_Task(self.sleep(),self.sleep_duration,self.house_location,'sleep')
                    if self.success == 2:
                        self.reward = self.age + self.net_worth + self.energy + self.hunger
                        self.update_q_value(self.action, self.reward)
                elif self.action == 'Go_to_convience_store':
                    self.Do_Task(self.Go_to_convience_store(),self.shopping_duration,self.Convience_Store_location,'sleep')
                    if self.success == 2:
                        self.reward = self.age + self.net_worth + self.energy + self.hunger
                        self.update_q_value(self.action, self.reward)
                elif self.action == 'Eat':
                    self.Do_Task(self.Eat(), 5,self.house_location,'wating')
                    if self.success == 2:
                        self.reward = self.age + self.net_worth + self.energy + self.hunger
                        self.update_q_value(self.action, self.reward)
