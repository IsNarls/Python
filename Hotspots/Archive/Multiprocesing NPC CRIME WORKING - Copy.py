        ...
        while self.is_alive == True:
            ...
            if self.action_lock == True:
                if self.action == 'work':
                    ...
                    self.Do_Task(self.Eat(), 5,self.house_location,'Eating')
                    if self.success == 2:
                        self.reward = self.age + self.net_worth + self.energy + self.hunger
                        self.update_q_value(self.action, self.reward)
...
def main():
    total_instances = 60
    processes = 60 # Adjust this number based on your system's capabilities

    # Create a pool of worker processes
    with multiprocessing.Pool(processes=processes) as pool:
        # Start the NPC_Crime instances in batches
...
"""if __name__ == "__main__":
    length = 60
    # Create a multiprocessing pool with 2 processes (adjust as needed)
    pool = multiprocessing.Pool(processes=length)


    # Start the continuous stock price monitoring for each symbol
    # The pool will manage the processes
    for i in range(60):
        person = NPC_Crime()
        pool.apply_async(person.update)
    ...
    #p.start()
    # Keep the main process alive (otherwise, it will exit immediately)
    while True:
        if main_counter == 100:
            #print('still going')
            main_counter = 0
